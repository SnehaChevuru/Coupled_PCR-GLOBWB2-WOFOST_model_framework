...
class LandCover(object):
    ...
    def initialize_imposed_fluxes(self, imposed_flux_names,  \
                                  missing_value):

        '''

initialize_imposed_fluxes: function that initializes the imposed fluxes as a \
spatially distributed map of missing values that will be used to \
...
'''
        ...
        for imposed_flux_name in imposed_flux_names:

            setattr(self, 'target_%s' % imposed_flux_name, \
                    pcr.ifthen(self.landmask, pcr.scalar(missing_value)))

            logger.debug('target value of %s for %s initialized' % \
                        (imposed_flux_name, self.name))

        # returns None
        return None

    def impose_target_flux(self, imposed_flux_name, imposed_flux_value, \
                           missing_value):

        '''
sets the target flux

'''

        if imposed_flux_name in self.imposed_flux_names:

            imposed_flux_value = pcr.cover(pcr.scalar(imposed_flux_value), \
...
#~
    ...
    def calculateWaterDemand(self, nonIrrGrossDemandDict, \
        ...
        # here, add the computation of the irrigation water requirements
