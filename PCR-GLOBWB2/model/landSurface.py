...
import model.landCover as lc
import model.parameterSoilAndTopo as parSoilAndTopo
...
class LandSurface(object):
    ...
    def initialize_imposed_landcover_variables(self):

        '''

initialize_imposed_landcover_fluxes: function that initializes the imposed \
fluxes as a spatially distributed map of missing values that will be used to \
set and update fluxes in the land cover update.
Values are set across all land cover types and for all the names that are \
contained by the list of names imposed_land_cover_flux_names for that land \
cover type and to the missing value identifier specified there.

'''

        # initialize the imposed fluxes
        for coverType in self.coverTypes:

            
            imposed_flux_names  = self.landCoverObj[coverType].imposed_flux_names
            imposed_state_names = self.landCoverObj[coverType].imposed_state_names
            missing_value       = self.landCoverObj[coverType].missing_value

            self.landCoverObj[coverType].initialize_imposed_variables( \
                                                    imposed_flux_names, \
                                                    missing_value)
            self.landCoverObj[coverType].initialize_imposed_variables( \
                                                    imposed_state_names, \
