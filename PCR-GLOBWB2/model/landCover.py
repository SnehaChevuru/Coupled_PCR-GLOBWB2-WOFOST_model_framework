...
from model.ncConverter import *

from model.crop_irrigation_functions import estimate_paddy_infiltration_capacity, \
                                      get_irrigation_water_requirement


#############
# functions #
#############
def scale_to_target_value(value_keys, \
                          original_values, \
                          default_values, \
...
scale_to_target_values: function that iterates over a list of names and uses \
the total of the original value, or if the total is zero, the default values, \
to compute a ratio and use this to redistribute the target value passed.

    Input:
    ======
    value_keys          : list of uniquestrings in the correct order to sort
                          the output of values; this list is used together with
                          the sequences of the original and default values to
                          create dictionaries for processing;
    original_values,
    default_values      : sequence (tuple, list) of the original value or the
                          default value that should be considered if the total
                          is zero; this can be either a sequence of floats or
                          a sequence of spatial scalar PCRaster fields but only
                          one type per variable is allowed; at least one entry
                          of the default values should be larger than zero.
    target_value        : target value that should be met by the total of the
                          returned values; the target value is expected to be
                          a spatial scalar PCRaster field. The output of all
                          values is clipped to the extent of the target_value
                          (e.g., landmask);
    missing_value       : missing value identifier, float.

    Output:
    =======
    values              : list of the broken down target value in the order of
                          the value_keys.

IMPORTANT: this function assumes that all entries are positive or zero;
           target value equal to the missing value identifier are are assumed to be not specified

'''

    # set list with None and for the number of keys as the iniital output of values
    # and get the mask for processing
    values = [None for key in value_keys]
    mask   = target_value != missing_value

    # create the dictionaries
    original_values = dict(zip(value_keys, original_values))
    default_values  = dict(zip(value_keys, default_values))
    target_values   = dict(zip(value_keys, values))

    # make all values scalar PCRaster maps and cover them with zero when no
    # data is available
    for value_key in value_keys:

        original_values[value_key] = pcr.max(0, pcr.cover( \
                        pcr.spatial(pcr.scalar(original_values[value_key])), 0))
        default_values[value_key] = pcr.max(0, pcr.cover( \
                        pcr.spatial(pcr.scalar(original_values[value_key])), 0))

    # get the totals
    total_original_values = sum(list(original_values.values()))
    total_default_values  = sum(list(default_values.values()))

    # making sure the default is not zero
    value_key                 = value_keys[0]
    default_values[value_key] = pcr.ifthenelse(total_default_values > 0, \
                                               default_values[value_key], \
                                               pcr.scalar(vos.smallNumber))
    total_default_values      = pcr.ifthenelse(total_default_values > 0, \
    ...
    for value_key in value_keys:
        ...
        # compute the fraction of the current value_key to the total
        # this is akin to vos.getValDivZero but makes it spatially variable
        # and layer depedndent
        scale_ratio = pcr.ifthenelse(total_original_values > 0, \
                                     pcr.min(1.0, original_values[value_key] / \
                                                  total_original_values), \
                                     pcr.min(1.0, default_values[value_key] / \
                                                  total_default_values))
        target_values[value_key] = pcr.ifthenelse(mask, \
                                                  scale_ratio * target_value, \
                                                  original_values[value_key])

        values[value_keys.index(value_key)] = target_values[value_key]
...
class LandCover(object):
    ...
    def __init__(self,iniItems,nameOfSectionInIniFile,soil_and_topo_parameters,landmask,irrigationEfficiency,usingAllocSegments = False):
        ...
        self.default_date_in_str = iniItems.globalOptions['startTime']
        ...
        # these are  variables that are initialized here but could be set in
        # the init section or parameterized (RvB);
        # a low value of the effective depth of evaporation is used
        # to account that transpiration and soil evaporation initially
        # compete for the same amount of water
        # the cropKC_tolerance is used to check on the growth stages
        # relative to the actual crop coefficient;
        # the max number of days is used to track crop development and schedule
        # the first irrigation
        self.cropKC_day_number_max            = 10.0
        self.cropKC_weight                    = 1.0 / self.cropKC_day_number_max
        self.cropKC_tolerance                 = 0.10
        self.maxCropKC                        = 1.20
        self.minimum_root_depth               = 0.05
        self.effective_depth_evaporation      = 0.10
        self.cropKC_maturity                  = 0.75
        self.default_irrigation_probability   = 0.80
        self.threshold_irrigation_probability = 0.90

        # the following three fields are needed to follow crop development
        # and include the average crop coefficient over the past N days,
        # the growth direction (1, 0, -1), and the number of days
        # TODO: replace with initial values
        self.avgCropKC                   =  0.20
        self.growth_direction            =  0
        self.crop_stage_day_number       =  0
        self.crop_day_number             =  0
        self.irrigation_probability      =  self.default_irrigation_probability
        ...
        if self.includeIrrigation:
            ...
            if 'irrigationType' in self.iniItemsLC.keys():

                self.irrigationType = self.iniItemsLC['irrigationType']

                assert self.irrigationType in ['irrPaddy', 'irrNonPaddy'], \
                       'irrigation type %s is not allowed' % self.irrigationType

                logger.info('irrigation type %s set for %s' % \
                            (self.irrigationType, self.name))

            else:

                self.irrigationType = None

            if not isinstance(self.irrigationType, vos.NoneType):
                # read in any other irrigation properties
                # cropDeplFactor (dimesionless, crop depletion factor while
                # irrigation is being applied), needed for NON paddy irrigation areas
                self.cropDeplFactor = vos.readPCRmapClone( \
                                          self.iniItemsLC['cropDeplFactor'], \
                                          self.cloneMap, self.tmpDir, self.inputDir)

                # infiltration capacity of paddy fields, based on ksat [m/day]
                if self.numberOfLayers == 2:\
                    ksat = self.parameters.kSatUpp
                if self.numberOfLayers == 3:\
                    ksat = self.parameters.kSatUpp000005

                if self.irrigationType == 'irrPaddy':

                    self.paddy_infiltration_capacity = \
                               estimate_paddy_infiltration_capacity(ksat)
                else:

                    self.paddy_infiltration_capacity = ksat
        ...
        # set the names of the imposed flux names, these are initialized
        # as missing values and can be updated elsewhere, thus modifying the
        # behaviour of the landCover module accordingly.
        self.imposed_flux_names = ['totalPotET', 'potBareSoilEvap', \
                                   'actBareSoilEvap', \
                                   'actTranspiration', 'openWaterEvap']

        self.imposed_state_names = ['root_depth']
    ...
    def get_depth_average(self, \
        ...
        # returns the depth average value

        value            = pcr.ifthen(self.landmask, pcr.scalar(0))
        cumulative_depth =  pcr.ifthen(self.landmask, pcr.scalar(0))

        for ix in range(len(layer_depths)):

            value = value + pcr.min(layer_depths[ix], \
                                    pcr.max(0, depth - cumulative_depth)) * \
                            value_list[ix]

            cumulative_depth = cumulative_depth + value

        # average the value
        value = vos.getValDivZero(value, depth)
    ...
    def get_land_cover_parameters(self, date_in_string = None, get_only_fracVegCover = False):
        ...
        if date_in_string == None:
            ...
            if self.iniItemsLC['landCoverMapsNC'] == str(None):
                # using pcraster maps or specific netCDF files
                landCoverPropertiesNC = None
                for var in landCovParams:

                    # deal with different input formats
                    if str(var) in self.iniItemsLC.keys():
                        input = self.iniItemsLC[str(var)]
                    elif str(var+'NC') in self.iniItemsLC.keys():
                        input = self.iniItemsLC[str(var+'NC')]
                    else:
                        logger.error('iformation on %s cannot be read' % var)

                    # check on the netCDF file condition
                    if input.endswith(vos.netcdf_suffixes):
                        
                        # get a date
                        if isinstance(date_in_string, type(None)):                           
                            def_date_in_str = self.default_date_in_str
                        else:
                            def_date_in_str = date_in_str

                        # get the param from a timed netCDF file                        
                        netcdf_file = vos.getFullPath(input, self.inputDir)
                        lc_parameters[var] = pcr.cover(
                                             vos.netcdf2PCRobjClone(netcdf_file,var, \
                                                                    def_date_in_str, useDoy = 'yearly',\
    ...
    def initialize_imposed_variables(self, imposed_variable_names,  \
                                  missing_value):

        '''

initialize_imposed_variables: function that initializes the imposed variables \
as a spatially distributed map of missing values that will be used to \
set and update fluxes in the land cover update.
Values are set for all the names that are contained by the list of names \
imposed_land_cover_flux_names to the missing value identifier specified \
for the land cover at hand; returns None.

'''

        for imposed_variable_name in imposed_variable_names:

            setattr(self, 'target_%s' % imposed_variable_name, \
                    pcr.ifthen(self.landmask, pcr.scalar(missing_value)))

            logger.debug('target value of %s for %s initialized' % \
                        (imposed_variable_name, self.name))

        # returns None
        return None

    def impose_target_variable(self, imposed_variable_name, imposed_variable_value, \
                           missing_value):
...
'''
        ...
        if imposed_variable_name in self.imposed_flux_names or \
                imposed_variable_name in self.imposed_state_names:

            imposed_variable_value = pcr.cover(pcr.scalar(imposed_variable_value), \
                                           pcr.scalar(missing_value))

            setattr(self, 'target_%s' % imposed_variable_name, \
                                        imposed_variable_value)

            logger.debug('target value of %s for %s set' % \
                        (imposed_variable_name, self.name))
    ...
    def updateLC(self,meteo,groundwater,routing,\
        ...
        # calendar #
        ############
        # crop specific function to keep track of the crop calendar;
        # this section has been added for irrigation purposes and it could be
        # made specific for crops only (RvB, Sepbtember 2023)

        # check on the crop calendar
        active_crop = self.cropKC >= (1.0 + self.cropKC_tolerance) * self.minCropKC

        # set the growth stage; is the crop coefficient above the maximum or is
        # the change very small? the threshold is set to half the threshold;
        # otherwise there is growth, decline or no growth.
        kc_slope = self.cropKC / self.avgCropKC - 1.0
        kc_slope = pcr.ifthenelse(2.0 * pcr.abs(kc_slope) <= self.cropKC_tolerance, \
                       pcr.scalar(1.0), 0.0) * pcr.ifthenelse(kc_slope < 0, \
                       pcr.scalar(-1), 1)
        kc_level = pcr.ifthenelse(2.0 * pcr.abs(self.cropKC / \
                                  pcr.max(self.maxCropKC, self.avgCropKC) - \
                                  1.0) <= self.cropKC_tolerance, pcr.scalar(1), \
                                  pcr.scalar(0))
        kc_slope = pcr.ifthenelse(active_crop, kc_slope, 0)
        kc_level = pcr.ifthenelse(active_crop, kc_level, 0)
        new_growth_direction = pcr.ifthenelse(kc_level == 1, pcr.ordinal(2), \
                                  pcr.ifthenelse(kc_slope != 0, pcr.ordinal(kc_slope), \
                                  self.growth_direction))
        new_growth_direction = pcr.ifthenelse(active_crop, new_growth_direction, 0)

        # update the growth stage and day numbers
        self.crop_day_number       = pcr.ifthenelse(active_crop, \
                                          self.crop_day_number + pcr.scalar(1), \
                                          pcr.scalar(0))
        self.crop_stage_day_number =  pcr.ifthenelse(active_crop & \
                                          (new_growth_direction == self.growth_direction),\
                                          self.crop_stage_day_number + pcr.scalar(1), \
                                          pcr.scalar(0))

        # update the growth direction and the average crop factor
        self.avgCropKC = pcr.ifthenelse(new_growth_direction != self.growth_direction, \
                                        self.cropKC, self.cropKC_weight * self.cropKC + \
                                        (1.0 - self.cropKC_weight) * \
                                        self.avgCropKC)
        self.avgCropKC = pcr.ifthenelse(active_crop, self.avgCropKC , self.minCropKC)
        self.growth_direction = new_growth_direction

        # end of crop calendar

        # set the soil states
        self.getSoilStates()

        # estimate the root depth
        self.root_depth = self.minimum_root_depth + \
                          pcr.max(0, self.maxRootDepth - self.minimum_root_depth) * \
                          pcr.ifthenelse(active_crop, \
                              pcr.min(1.00 , \
                              pcr.max(0, self.cropKC - self.minCropKC) /\
                              pcr.max(0.01, self.maxCropKC - self.minCropKC)) ** 0.5, 0)

        self.root_depth= pcr.ifthenelse( \
                              self.target_root_depth != pcr.scalar(self.missing_value), \
                              self.target_root_depth, self.root_depth)

        self.root_depth = pcr.min(self.root_depth, self.soil_depth)
    ...
    def getPotET(self, meteo, currTimeStep):
        ...
        # target values: check on the potential potential evapotranspiration
        # and the potential bare soil evaporation; because of the water balance
        # check, the potential transpiration is not updated
        self.totalPotET = pcr.ifthenelse( \
                              self.target_totalPotET != pcr.scalar(self.missing_value), \
                              self.target_totalPotET, self.totalPotET)

        self.potBareSoilEvap = pcr.ifthenelse( \
                              self.target_potBareSoilEvap != pcr.scalar(self.missing_value), \
                              self.target_potBareSoilEvap, self.potBareSoilEvap)

        # the potential transpiration, prior to root water uptake, is all
        # potential evapotranspiration minus the potential bare soil evaporation
...
#~
    ...
    def getSoilStates(self):
        ...
        if self.numberOfLayers == 2:
            ...
            self.soil_depth = self.parameters.thickUpp + self.parameters.thickLow
        ...
        if self.numberOfLayers == 3:
            ...
            self.soil_depth = self.parameters.thickUpp000005 + self.parameters.thickUpp005030 + self.parameters.thickLow030150
    ...
    def calculateWaterDemand(self, nonIrrGrossDemandDict, \
        ...
        # set the initial estimate of the irrigation water requirement, this
        # is only updated if the crop is irrigated
        self.irrigation_water_requirement = pcr.ifthen(self.landmask, pcr.scalar(0))
        self.irrGrossDemand               = pcr.ifthen(self.landmask, pcr.scalar(0))
        self.irrGrossDemandPaddy          = pcr.ifthen(self.landmask, pcr.scalar(0))
        self.irrGrossDemandNonPaddy       = pcr.ifthen(self.landmask, pcr.scalar(0))
        ...
        if self.includeIrrigation and \
                not isinstance(self.irrigationType, vos.NoneType):

            # there are two possible irrigation modes: there is early paddy
            # and irrigation according to the FAO scheme; in addition, the
            # first irrigation application is different and irrigation need has
            # to be found.
            # these conditions are administrated using boolean maps
            allow_irrigation = pcr.ifthen(self.landmask, \
                                          pcr.cover(self.fracVegCover > 0, 0))
            early_irrigation = pcr.ifthen(self.landmask, pcr.boolean(0))
            paddy_irrigation = pcr.ifthen(self.landmask, pcr.boolean(0))

            # update these maps
            # first get the water stress factor and decide whether there
            # is need for irrigation
            actual_evapotranspiration_estimate = \
                       self.estimateTranspirationAndBareSoilEvap(returnTotalEstimation = True)
            potential_evapotranspiration_estimate = self.potBareSoilEvap + \
                       self.potTranspiration
            water_stress_ratio = pcr.max(0, 1.0 - \
                                         vos.getValDivZero(actual_evapotranspiration_estimate, \
                                         potential_evapotranspiration_estimate))
            allow_irrigation = (self.crop_day_number > 0) & \
                              (potential_evapotranspiration_estimate > 1.0e-4)

            # is this early irrigation?
            self.irrigation_probability = pcr.ifthenelse((self.crop_day_number > 0) & \
                             (self.crop_day_number <= self.cropKC_day_number_max),
                             0.5 * (self.irrigation_probability + \
                                    pcr.exp(- water_stress_ratio / \
                                           pcr.max(1, self.crop_day_number))), \
                             pcr.scalar(self.default_irrigation_probability))
            early_irrigation = self.irrigation_probability >= \
                               self.threshold_irrigation_probability

            # is this paddy irrigation?
            if self.irrigationType == 'irrPaddy':

                # note the crop factor at maturity  condition is exclueded:
                # (self.cropKC < self.cropKC_maturity
                paddy_irrigation = allow_irrigation & \
                      pcr.pcrnot((self.crop_day_number > 60) & \
                                 (self.growth_direction == -1))

            # all decision variables set, next compute the relevant parameters
            # on the basis of the root depth
            irrigation_root_depth = pcr.max(self.minimum_root_depth, \
                        self.root_depth, self.effective_depth_evaporation)

            if self.numberOfSoilLayers == 2:

                layer_depths = [self.parameters.thickUpp, \
                                self.parameters.thickLow]

                theta_sat = self.get_depth_average(irrigation_root_depth, \
                            layer_depths, \
                            [self.parameters.satVolWC1, \
                             self.parameters.satVolWC2])
                theta_res = self.get_depth_average(irrigation_root_depth, \
                            layer_depths, \
                            [self.parameters.resVolWC1, \
                             self.parameters.resVolWC2])

                theta     = self.get_depth_average(irrigation_root_depth, \
                            layer_depths, \
                            [self.effSatUpp * (self.parameters.satVolWC1 - self.parameters.resVolWC1) + \
                             self.parameters.resVolWC1, \
                             self.effSatLow *  (self.parameters.satVolWC2 - self.parameters.resVolWC2) + \
                             self.parameters.resVolWC2])
                theta_fc  = self.get_depth_average(irrigation_root_depth, \
                            layer_depths, \
                            [self.parameters.effSatAtFieldCapUpp * \
                                 (self.parameters.satVolWC1 - self.parameters.resVolWC1) + \
                                 self.parameters.resVolWC1, \
                             self.parameters.effSatAtFieldCapLow * \
                                 (self.parameters.satVolWC2 - self.parameters.resVolWC2) + \
                                 self.parameters.resVolWC2])
                theta_wp  = self.get_depth_average(irrigation_root_depth, \
                            layer_depths, \
                ...
                ksat = pcr.ifthenelse(irrigation_root_depth < self.parameters.thickUpp, \
                           self.parameters.kSatUpp, self.parameters.kSatLow)

            else:

                layer_depths = [self.parameters.thickUpp000005, \
                ...
                ksat = pcr.ifthenelse(irrigation_root_depth < self.parameters.thickUpp000005, \
                           self.parameters.kSatUpp000005, \
                           pcr.ifthenelse(irrigation_root_depth < self.parameters.thickUpp005030, \
                               self.parameters.kSatLow030150, self.parameters.kSatLow030150))

            # TODO: consider possibly percolation losses for non-paddy, based on k(theta)
            percolation_loss = ksat * (theta_fc - theta_res) / (theta_sat - theta_res)
            percolation_loss = 0
            ...
            self.paddy_irrigation = paddy_irrigation

            # get the irrigagion water demand [m/day over irrigated area]
            self.irrigation_water_requirement = get_irrigation_water_requirement( \
                    irrigation_type             = self.irrigationType, \
                    min_water_layer_depth       = self.minTopWaterLayer, \
                    depletion_factor            = self.cropDeplFactor, \
                    irrigation_efficiency       = self.irrigationEfficiencyUsed, \
                    effective_depth_evaporation = self.effective_depth_evaporation, \
                    root_depth                  = irrigation_root_depth, \
                    percolation_loss            = percolation_loss, \
                    paddy_infiltration_capacity = self.paddy_infiltration_capacity, \
                    potential_evaporation       = self.cropKC * self.totalPotET, \
                    open_water_evaporation      = self.totalPotET, \
                    liquid_water_input          = self.netLqWaterToSoil, \
                    water_layer_depth           = self.topWaterLayer, \
                    theta_sat                   = theta_sat, \
                    theta_res                   = theta_res, \
                    theta_fc                    = theta_fc, \
                    theta_wp                    = theta_wp, \
                    ...
                    early_irrigation            = early_irrigation, \
                    paddy_irrigation            = paddy_irrigation)

        # set the irrigation gross demand
        self.irrGrossDemand = self.irrigation_water_requirement

        if self.irrigationType == 'irrPaddy':
            self.totalPotentialMaximumIrrGrossDemandPaddy = self.irrGrossDemand
        elif self.irrigationType == 'irrNonPaddy':
            self.totalPotentialMaximumIrrGrossDemandNonPaddy = self.irrGrossDemand
        else:
            pass

        # add this point, the irrigation water requirements are set.
        ...
        if self.irrigationType == 'irrNonPaddy':
    ...
    def calculateOpenWaterEvap(self):
        ...
        if self.name == 'irrPaddy' or self.name == "irr_paddy":  # only open water evaporation from the paddy field
            self.openWaterEvap = remainingPotETP

        # target value, set the open water evaporation to the target
        # value and then scale it to the availability
        self.openWaterEvap = pcr.ifthenelse(self.target_openWaterEvap != self.missing_value, \
                                            self.target_openWaterEvap, self.openWaterEvap)

        # open water cannot be larger than the top water layer
        self.openWaterEvap = pcr.min(pcr.max(0.,self.topWaterLayer), \
                                     self.openWaterEvap)
    ...
    def calculateInfiltration(self):
        ...
        if self.irrigationType == 'irrPaddy' and self.includeIrrigation:
            # infiltration_loss = pcr.max(self.paddy_infiltration_capacity, \
            #                     ((1./self.irrigationEfficiencyUsed) - 1.) * self.topWaterLayer)
            # self.infiltration = pcr.min(infiltration_loss, self.infiltration)
            self.infiltration = pcr.min(self.paddy_infiltration_capacity, self.infiltration)
    ...
    def estimateTranspirationAndBareSoilEvap(self, returnTotalEstimation = False, returnTotalTranspirationOnly = False):
        ...
        if self.numberOfLayers == 2:
            ...
            # compute the values per layer, and obtain the total
            ...
            # target values: update the actual transpiration per layer
            actTranspiUpp, actTranspiLow = scale_to_target_value( \
                                                 value_keys =      ['layer1', 'layer2'], \
                                                 original_values = (actTranspiUpp, \
                                                                    actTranspiLow), \
                                                 default_values  = (0.30, 0.0), \
                                                 target_value    = self.target_actTranspiration, \
        ...
        if self.numberOfLayers == 3:
            ...
            # target values: update the actual transpiration per layer
            actTranspiUpp000005, actTranspiUpp005030,\
                actTranspiLow030150      = scale_to_target_value( \
                                                 value_keys =      ['layer1', 'layer2', 'layer3'], \
                                                 original_values = (actTranspiUpp000005, \
                                                                    actTranspiUpp005030,\
                                                                    actTranspiLow030150), \
                                                 default_values  = (0.05, 0.30, 0.0), \
                                                 target_value    = self.target_actTranspiration, \
                                                 missing_value   = self.missing_value)
        ...
        # target values: check on the actual bare soil evaporation
        actBareSoilEvap = pcr.ifthenelse( \
                              self.target_actBareSoilEvap != pcr.scalar(self.missing_value), \
                              self.target_actBareSoilEvap, actBareSoilEvap)
...
###############################################################################
        ...
        #     # self.test_irrig_demand = self.irrGrossDemand
